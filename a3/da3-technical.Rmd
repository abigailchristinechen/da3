...
## Check This ->Add a summary like this 
...
## Random Forest
The last model we created is the probabiloty Random Forest, where we will be using the set of variables from the random forest modelling. The variables we will be using are the raw variables, firm variables and HR variables. The result from the Random Forest shows that the probability random forest 

```{r echo=FALSE, message=FALSE, warning=FALSE}
rf_summary
```
The result from the Random Forest indicates that (probability) Random Forest outperforms other models such as LASSO and probability logit by around -0.001 RMSE and the AUC result  is higher than the both mentioned model 3 and LASSO logit. As a result the Random Forest based on the RMSE and AUC result is the best performing model. Thus, we use our holdout set to draw ROC curve.
...
summary_results <- summary_results %>%
  filter(rownames(.) %in% c("X1", "X2","X3","X4", "LASSO", "rf_p"))
...
Here we can see that the Random Forest has the lowest expected loss and RMSE, compared to the other modells.  
